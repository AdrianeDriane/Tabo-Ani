...
description: Implement or modify React + TypeScript frontend functionality in client/ using existing project patterns, production-ready code, and the file organization defined in client/FILE_STRUCTURE.md.
---

# Purpose

Use this skill when working on frontend functionality in `client/`.
...
- updating state handling, forms, tables, modals, and user interactions
...
1. Inspect existing frontend architecture.
   - Review relevant pages, components, hooks, services, store usage, DTOs, interfaces, and utility patterns.
   - Review `client/FILE_STRUCTURE.md` before creating or moving files.
...
4. Preserve architectural consistency.
   ...
   - Avoid introducing broader state changes unless clearly necessary.

5. Validate consistency.
   - Ensure success/error handling aligns with backend response structure.
   - Ensure types, DTOs, and interfaces stay aligned with backend contracts.
...
- Follow existing state management patterns such as Redux or the current store setup.
...
- Implement DTOs, interfaces, and types where appropriate.
- Keep frontend contracts aligned with backend request/response shapes.

## Error Handling and Feedback

- Have proper UI toast error handling where applicable.
...
- place them according to `client/FILE_STRUCTURE.md`
...
- Do not introduce global state changes unless necessary.
- Do not ignore `client/FILE_STRUCTURE.md`.
...
- placed correctly according to `client/FILE_STRUCTURE.md`
