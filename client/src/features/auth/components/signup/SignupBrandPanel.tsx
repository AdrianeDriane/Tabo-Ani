...
export function SignupBrandPanel({ content }: SignupBrandPanelProps) {
  return (
    <aside className="relative hidden h-full min-h-0 overflow-hidden bg-agri-green p-8 text-white md:flex md:w-5/12 md:flex-col md:justify-between md:p-10 lg:w-1/2 lg:p-12">
      <div
        className={`absolute inset-0 ${content.imageOpacityClassName ?? ""}`}
      >
        <img
      ...
      <div className="relative z-10">
        ...
        <h1 className="mb-4 font-display text-3xl leading-tight font-extrabold lg:mb-6 lg:text-5xl">
          {content.heroTitle}
        </h1>
        <p className="max-w-md text-base leading-relaxed text-white/90 lg:text-lg">
          {content.heroDescription}
        </p>
      </div>

      {content.highlights || content.testimonial || content.footerNote ? (
        <div className="relative z-10 mt-auto space-y-6 lg:space-y-8">
          {content.highlights ? (
          ...
          {content.testimonial ? (
            <div className="rounded-3xl border border-white/20 bg-white/10 p-5 backdrop-blur-md lg:p-6">
              <div className="mb-3 flex gap-1 text-agri-accent">
                {Array.from(
                  { length: content.testimonial.stars ?? 5 },
                  (_, index) => (
                    <span
                      key={index}
              ...
              <div className="flex items-center gap-3">
                <div
                  ...
                  style={{
                    backgroundImage: `url(${content.testimonial.avatarUrl})`,
                  }}
                />
                <div>
          ...
          {content.footerNote ? (
            <div className="text-xs font-medium text-white/70 lg:text-sm">
              {content.footerNote}
            </div>
          ) : null}
        </div>
